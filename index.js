...
let apiCall = async () => {
    ...
    let request = await fetch("https://api.giphy.com/v1/gifs/random?api_key=fwGiVtydJOpg64tS7EWm4tfwjYcOXZg0&tag=simpsons&rating=g");

    let response = await request.json();
        

    let gifUrl = await response.data.images.downsized_medium.url;
...
class GifTV
{
	constructor( channels = ['https://res.cloudinary.com/cyborgspaceviking/image/upload/v1571155222/giphy_n0r827.gif'] )
	{
		...
		this.currentChannelURL = 'https://res.cloudinary.com/cyborgspaceviking/image/upload/v1571155222/giphy_n0r827.gif';
	...
	displayChannel()
	{
		apiCall().then(gif => this.gifVideo.setAttribute("src", gif))
	...
	init()
	{
		...
		// Volume Dial
		this.dial.volume.button.addEventListener( 'click', (e) => { e.preventDefault(); this.changeVolume( 'up' ); })
		this.dial.volume.button.addEventListener( 'contextmenu', () => { this.changeVolume( 'down' ) })
		...
		document.onkeydown = ( e ) =>
		{
			...
			// M key
			e.keyCode == '77' ? this.toggleMute() : null
			// H key
			e.keyCode == '72' ? this.toggleHighDef() : null
			// S key
			e.keyCode == '83' ? this.toggleHueShift() : null
			// D key
			e.keyCode == '68' ? this.toggleBright() : null
			// B key
			e.keyCode == '66' ? this.toggleColor() : null
		}
	}
}
